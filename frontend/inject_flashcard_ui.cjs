...
// Inject state and toggle function inside TestReview
const hookRegex = /const \{ subject, chapter, result \} = reviewState/g;
...
// Inject button into the question card header
const renderRegex = /<h3 className="review-question-title">/;
code = code.replace(renderRegex, `<div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <h3 className="review-question-title" style={{ flex: 1 }}>`);
                
const renderEndRegex = /<\/h3>/g;
code = code.replace(renderEndRegex, `</h3>
                <button 
                  type="button" 
